...
/**
 * Created by lq on 16/11/22.
 * 图片压缩工具类
 * 此处压缩为质量压缩和尺寸压缩。需用JNI重新调用哈夫曼算法开发
 */
public class BitmapUtils {
        ...
        public static Bitmap compressByQuality(Bitmap bitmap, int maxSize) {
            ...
            Log.e("BitmapUtils","图片压缩前大小：" + baos.toByteArray().length + "byte");
            boolean isCompressed = false;
            while (baos.toByteArray().length / 1024 > maxSize && quality !=10) {
                quality -= 10;
                baos.reset();
                bitmap.compress(Bitmap.CompressFormat.PNG, quality, baos);
                Log.e("BitmapUtils","质量压缩到原来的" + quality + "%时大小为："
                        + baos.toByteArray().length + "byte");
                isCompressed = true;
            }
            Log.e("BitmapUtils","图片压缩后大小：" + baos.toByteArray().length + "byte");

            System.out.println("图片压缩后大小：" + baos.toByteArray().length + "byte"+"  isCompressed: "+isCompressed+"");
            if (isCompressed) {
